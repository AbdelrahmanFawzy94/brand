...
export class CustomValidation {
  ...
  static startWithSpace(control: AbstractControl): ValidationErrors | null {
    ...
    if (spaceRegex.test(val)) {
      return {
        startWithSpace: true,
        message: 'this field cannot start with space',
      };
    }
    return null;
  }
  ...
  static isValidPassport(control: AbstractControl): ValidationErrors | null {
    const val = control.value as string;
    // const spaceRegex = /^[A-PR-WY][1-9]\d\s?\d{4}[1-9]$/gi;
    // const spaceRegex = /^[A-PR-WYa-pr-wy][1-9]\d\s?\d{4}[1-9]$/;
   ...
   * @returns `boolean`
   */
  static isArabicLetters(control: AbstractControl): ValidationErrors | null {
  ...
  /**
   * @description `isenglishonly` check if the value is valid Arabic letter and numbers [ا-ي] [0-9]
   * @param `control` string
   * @returns `boolean`
   */
  static isArabicPattern(control: AbstractControl): ValidationErrors | null {
    const val: string = control.value as string;
  ...
  static isAlpha(control: AbstractControl): ValidationErrors | null {
    ...
    if (!validator.isAlpha(val === null ? '' : val)) {
      return { alpha: true };
    }
    return null;
  }
   ...
   * @returns `boolean`
   */
  static isEnglishLetters(control: AbstractControl): ValidationErrors | null {
    const val: string = control.value as string;
  ...
  /**
   * @description `isenglishonly` check if the value is valid english letter and numbers [A-Z] [0-9]
   * @param `control` string
   * @returns `boolean`
   */
  static isEnglishPattern(control: AbstractControl): ValidationErrors | null {
    const val: string = control.value as string;
